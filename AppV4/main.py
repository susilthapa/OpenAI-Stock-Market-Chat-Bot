...
def main():
    ...
    st.caption(
        "Visualizations and OpenAI Chatbot for Multiple Stocks Over A Specified Period")
    ...
    if st.session_state.user_authenticated:
        with st.sidebar:
            with st.expander("Settings", expanded=True):
                asset_tickers = sorted(['DOW', 'NVDA', 'TSL', 'GOOGL', 'AMZN', 'AI',
                                       'NIO', 'LCID', 'F', 'LYFY', 'AAPL', 'MSFT', 'BTC-USD', 'ETH-USD'])
                ...
                start = st.date_input(
                    'Start', value=pd.to_datetime('2023-01-01'))
